...
int main() try {
  auto autd = autd3::Controller::open(
      {autd3::AUTD3{.pos = autd3::Point3::origin(), .rot = autd3::Quaternion::Identity()},
       autd3::AUTD3{.pos = autd3::Point3(autd3::AUTD3::DEVICE_WIDTH, 0.0, 0.0), .rot = autd3::Quaternion::Identity()}},
      autd3::link::EtherCrab{
          [](const uint16_t idx, const autd3::link::Status status) { std::cout << std::format("Device[{}]: {}", idx, status) << std::endl; },
